...
<script>
import {
  databases, databaseId, songsCollectionId, commentsCollectionId, account,
...
export default {
  ...
  async beforeRouteEnter(to, from, next) {
    let docSnapshot;
    try {
      docSnapshot = await databases.getDocument(databaseId, songsCollectionId, to.params.id);
    } catch (e) {
      next((vm) => {
  ...
  methods: {
    ...
    async addComment(values, { resetForm }) {
      ...
      await databases.updateDocument(databaseId, songsCollectionId, this.$route.params.id, {
    ...
    async getComments() {
      const response = await databases.listDocuments(
        ...
        [Query.equal('sid', this.$route.params.id)],
      );
