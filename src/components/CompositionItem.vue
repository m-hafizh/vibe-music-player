...
export default {
  ...
  methods: {
    async edit(values) {
      ...
      try {
        await databases.updateDocument(databaseId, songsCollectionId, this.song.docID, values);
      } catch (error) {
    ...
    async deleteSong() {
      if (this.song.file_id) {
        try {
          await storage.deleteFile(storageBucketId, this.song.file_id);
        } catch (e) {
          // File might not exist or be corrupted, continue to delete document
        }
      }

      await databases.deleteDocument(databaseId, songsCollectionId, this.song.docID);
